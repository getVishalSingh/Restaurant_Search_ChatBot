...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", limit="" , offset=0):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&start="+str(offset)+"&count=20"+"&cuisines=" + str(cuisines)+"&sort=rating&order=desc", headers=headers).content).decode("utf-8")
        return r
